...
void KEngine::KSystem::RenderSystemFlawed::init(int renderTargetWidth, int renderTargetHeight)
{
	m_camera.setProjParameters((3.14f / 2.0f ), renderTargetWidth, renderTargetHeight, 0.1f, 1000.0f);
}

void KEngine::KSystem::RenderSystemFlawed::setCameraProjectionFOV(float ratio)
{
}

void KEngine::KSystem::RenderSystemFlawed::setCameraPosition(Vector3 position)
{
	m_camera.setPosition(position);
}

void KEngine::KSystem::RenderSystemFlawed::setCameraRotation(Quaternion rotation)
{
	m_camera.setQuaternion(rotation);
}


void RenderSystemFlawed::setMatrix(ISimpleShader * shader, std::string name, DirectX::XMMATRIX matrix) {

	DirectX::XMStoreFloat4x4(&matTemp, XMMatrixTranspose(matrix));
	shader->SetMatrix4x4(name, matTemp);
}

void RenderSystemFlawed::setRenderTarget(
...
void RenderSystemFlawed::renderMesh(ID3D11DeviceContext * context, Mesh & mesh, UINT offsetBegin, UINT stride)
{
...
void RenderSystemFlawed::render(
...
void RenderSystemFlawed::addEntity(KEngine::Entity &entity) {
...
void RenderSystemFlawed::update(float time)
{
}
Renderable & RenderSystemFlawed::getComponent(int n)
{
	return m_components[n];
}

void KEngine::KSystem::RenderSystemFlawed::OnResize(int targetFrameWidth, int targetFrameHeight)
{
