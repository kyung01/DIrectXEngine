	...
	std::map<KEnum, std::unique_ptr<Mesh*>> &meshes, std::map<KEnum, ID3D11ShaderResourceView*> &textures, ID3D11SamplerState * sampler)
{
	...
	texture_diffuse.clear(context, 0, 0.0, 0, 0);
	texture_normal.clear(context, 0, 0, 0, 0);
	texture_specular.clear(context, 0, 0, 0, 0);
	...
	ID3D11SamplerState * samplerDefault, ID3D11SamplerState * samplerLightDepth)
{
	...
	auto matScreenOrtho = getOrthogonalMatrixProj();
	...
	for (auto it = scene.objs_lights.begin(); it != scene.objs_lights.end(); it++) {
		...
		//orthogonal render
		DirectX::XMStoreFloat4x4(&projection, XMMatrixTranspose(DirectX::XMMatrixMultiply(getOrthogonalMatrixView(), matScreenOrtho))); // Transpose for HLSL!
	...
	ID3D11SamplerState * samplerDefault, ID3D11SamplerState * samplerLightDepth, ID3D11SamplerState * samplerLightRSM)
{
	...
	targetIndirectLight.clear(context, 1, 1, 0, 0);
	context->OMSetBlendState(m_blendStateNoBlack, 0, 0xffffffff);
	for (auto it = scene.objs_lights.begin(); it != scene.objs_lights.end(); it++) {
...
void GraphicMain::render(
	ID3D11Device * device, ID3D11DeviceContext *context, 
	RenderTexture &textureScreen, DepthTexture &depthScreen, 
...
void NGraphic::GraphicMain::render(ID3D11Device * device, ID3D11DeviceContext * context, Mesh & plane, ID3D11RenderTargetView * target, ID3D11DepthStencilView * targetDepth, D3D11_VIEWPORT & viewport)
{
}
